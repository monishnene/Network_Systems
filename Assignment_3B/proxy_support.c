...
int checkCacheFile(int8_t *url, uint32_t timeout)
{
  FILE *fp;
...
int checkCacheHost(int8_t *hostname, int8_t *ip){
  FILE* fp;
  int8_t * line=NULL;
  size_t length;
  int8_t  filename[MAXBUFSIZE];
...
int checkForbiddenHost(int8_t  *hostname, int8_t  *forbid_ip){
  FILE * fp;
  int8_t * line=NULL;
  size_t length;
  int flag = 0;
...
int linkPrefetch(int8_t * prefetch_ip, int8_t * filename, int8_t * hostname, int8_t * port ){
  FILE * fp1;
  FILE * fp2;
  int8_t * line = NULL;
  size_t length;
